...
import { STAGES, PLANNING_STATUSES, AUTO_PLANNING_STAGES } from '../lib/constants.js';
import {
  usePlanningTasks,
  createPlanningTask,
...
export default function Planning({ projects, allAreas, userName }) {
  ...
  async function autoGenerate() {
    ...
    // Check if tasks already exist for this area
    const existing = tasks.filter((t) => t.area_id === autoForm.area_id);
    const existingStages = existing.map((t) => t.stage);

    const newTasks = AUTO_PLANNING_STAGES
      .filter((s) => !existingStages.includes(s.id))
      .map((s, idx) => ({
        project_id: autoForm.project_id,
        area_id: autoForm.area_id,
        title: `${s.label} — ${area.name}`,
        stage: s.id,
        status: 'pending',
    ...
    if (newTasks.length === 0) {
      alert('Ya existen tareas para todos los procesos de esta área.');
      return;
    }
    ...
    try {
      ...
      await logActivity({
        ...
        description: `${newTasks.length} procesos generados para ${area.name} (${project.name})`,
        user_name: userName,
      });
  ...
  async function save() {
    ...
    const payload = {
      ...
      priority: editing ? editing.priority : tasks.length,
    };
    try {
      if (editing) {
        ...
        await logActivity({
          ...
          area_id: form.area_id || null,
          action: 'task_created',
          description: `Tarea creada: "${form.title}"`,
          user_name: userName,
        });
      }
      setShowModal(false);
  ...
  async function toggleStatus(task) {
    ...
    await logActivity({
      ...
      description: `"${task.title}" → ${PLANNING_STATUSES.find((s) => s.id === next)?.label}`,
      user_name: userName,
    });
  }
  ...
  async function handleDragEnd() {
    ...
    try {
      await reorderPlanningTasks(reordered.map((t) => t.id));
    } catch (e) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--t2)' }}>
            Arrastra las tareas para reorganizar el orden de trabajo
          </p>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <Btn variant="secondary" onClick={() => {
            setAutoForm({ project_id: projects[0]?.id || '', area_id: '' });
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={filterProject} onChange={(e) => setFilterProject(e.target.value)} style={{ ...selectStyle, width: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          {PLANNING_STATUSES.map((s) => (
            <FilterBtn key={s.id} active={filterStatus === s.id} onClick={() => setFilterStatus(s.id)} color={s.color}>
              {s.label}
            </FilterBtn>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon="📅"
          title="Sin tareas"
          description="Usa 'Auto-generar' para crear los procesos de un área, o agrega tareas manualmente."
          action={<Btn onClick={() => { setAutoForm({ project_id: projects[0]?.id || '', area_id: '' }); setShowAutoModal(true); }}>⚡ Auto-generar procesos</Btn>}
        />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {filtered.map((t, idx) => {
            const statusInfo = PLANNING_STATUSES.find((s) => s.id === t.status) || PLANNING_STATUSES[0];
            const stageInfo = t.stage ? STAGES.find((s) => s.id === t.stage) || AUTO_PLANNING_STAGES.find((s) => s.id === t.stage) : null;
            return (
              <div
                key={t.id}
                draggable
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                  ...
                  {/* Order number */}
                  <span style={{
                    background: 'var(--hover)', borderRadius: 4, padding: '2px 6px',
                    fontSize: 10, fontWeight: 700, color: 'var(--t2)', minWidth: 20, textAlign: 'center',
                    marginTop: 3,
                  }}>
                    {idx + 1}
                  </span>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--t1)' }}>{t.title}</span>
                    ...
                    <div style={{ display: 'flex', gap: 10, fontSize: 11, color: 'var(--t2)', flexWrap: 'wrap' }}>
                      {t.project_id && <span>📋 {projectMap[t.project_id] || ''}</span>}
                      {t.area_id && <span>📐 {areaMap[t.area_id] || ''}</span>}
                    </div>
                    {t.material && (
                      <div style={{ fontSize: 11, color: '#e6a23c', marginTop: 3 }}>
                        🪵 Material: {t.material}
                      </div>
                    )}
                    {t.description && (
                      <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 2 }}>{t.description}</div>
                    )}
                  </div>
      ...
      <Modal open={showAutoModal} onClose={() => setShowAutoModal(false)} title="Auto-generar procesos" width={440}>
        <p style={{ fontSize: 13, color: 'var(--t2)', marginBottom: 16, lineHeight: 1.5 }}>
          Genera automáticamente las tareas de producción (Modelado, Planos, Corte, Canteado, Mecanizado) para un área. Después puedes reordenarlas y agregar más.
        </p>
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 600, color: 'var(--t2)' }}>Proyecto</label>
          <select value={autoForm.project_id} onChange={(e) => setAutoForm({ project_id: e.target.value, area_id: '' })} style={selectStyle}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <select value={autoForm.area_id} onChange={(e) => setAutoForm({ ...autoForm, area_id: e.target.value })} style={selectStyle}>
            <option value="">Selecciona</option>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
          ...
          <Btn onClick={autoGenerate} disabled={!autoForm.project_id || !autoForm.area_id}>⚡ Generar</Btn>
        </div>
      </Modal>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? 'Editar tarea' : 'Nueva tarea'} width={520}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 600, color: 'var(--t2)' }}>Proyecto</label>
            <select value={form.project_id} onChange={(e) => setForm({ ...form, project_id: e.target.value, area_id: '' })} style={selectStyle}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 600, color: 'var(--t2)' }}>Área</label>
            <select value={form.area_id} onChange={(e) => setForm({ ...form, area_id: e.target.value })} style={selectStyle}>
              <option value="">Sin área</option>
              {areasForFormProject.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 600, color: 'var(--t2)' }}>Proceso</label>
        ...
        <InputField
          ...
          placeholder="Ej: MDF 18mm blanco, Melamina roble 15mm..."
          textarea
        />

        <InputField label="Notas adicionales" value={form.description} onChange={(v) => setForm({ ...form, description: v })} placeholder="Detalles, observaciones..." textarea />
