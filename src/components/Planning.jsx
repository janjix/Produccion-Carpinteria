      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {tasks.filter((t) => t.status === 'done').length > 0 && (
            <Btn variant="ghost" onClick={async () => { if (!confirm(`¿Eliminar ${tasks.filter((t) => t.status === 'done').length} tareas completadas?`)) return; await deleteCompletedPlanningTasks(); }}>🗑 Limpiar completadas</Btn>
          )}
