...
export default function Planning({ projects, allAreas, userName }) {
  ...
  async function cycleStatus(task) {
    ...
    if (next === 'done' && task.stage && task.project_id) {
      ...
      if (created.length > 0) {
        await logActivity({ project_id: task.project_id, action: 'auto_propagated', description: `${created.length} proceso(s) creado(s) automáticamente tras completar "${task.title}"`, user_name: 'Sistema' });
      }
    }
  }
