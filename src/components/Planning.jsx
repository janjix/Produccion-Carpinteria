...
export default function Planning({ projects, allAreas, userName }) {
  ...
  const [form, setForm] = useState({
    project_id: '', area_id: '', title: '', description: '',
    stage: '', status: 'pending', material: '',
  });
  ...
  async function autoGenerate() {
    ...
    try {
      ...
      if (newTasks.length === 0) {
        alert('Ya existen tareas para todos los procesos/materiales de esta área.');
      } else {
  ...
  function openNew() {
    setEditing(null);
    setForm({ project_id: projects[0]?.id || '', area_id: '', title: '', description: '', stage: '', status: 'pending', material: '' });
    setShowModal(true);
  }
  ...
  function openEdit(t) {
    setEditing(t);
    setForm({
      project_id: t.project_id || '', area_id: t.area_id || '', title: t.title,
      description: t.description || '', stage: t.stage || '', status: t.status, material: t.material || '',
    });
    setShowModal(true);
  }
  ...
  async function save() {
    ...
    try {
      if (editing) {
        ...
        await logActivity({ project_id: form.project_id, area_id: form.area_id || null, action: 'task_created', description: `Tarea creada: "${form.title}"`, user_name: userName });
      }
      setShowModal(false);
  ...
  async function toggleStatus(task) {
    ...
    await logActivity({ project_id: task.project_id, action: 'task_status_changed', description: `"${task.title}" → ${PLANNING_STATUSES.find((s) => s.id === next)?.label}`, user_name: userName });

    // Auto-propagate: when marked done, create successor tasks
    if (next === 'done' && task.stage && task.project_id) {
      ...
      if (created.length > 0) {
        await logActivity({
          ...
          description: `${created.length} proceso(s) creado(s) automáticamente tras completar "${task.title}"`,
          user_name: 'Sistema',
        });
      }
    }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--t2)' }}>Arrastra para reorganizar. Agrega materiales en cada área antes de auto-generar.</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={filterProject} onChange={(e) => setFilterProject(e.target.value)} style={{ ...ss, width: 'auto' }}>
          ...
          {projects.map((p, i) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon="📅" title="Sin tareas" description="Agrega materiales a tus áreas y usa 'Auto-generar' para crear los procesos." action={<Btn onClick={() => { setAutoForm({ project_id: projects[0]?.id || '', area_id: '' }); setShowAutoModal(true); }}>⚡ Auto-generar</Btn>} />
      ) : (
            ...
            return (
              <div key={t.id} draggable onDragStart={() => handleDragStart(idx)} onDragEnter={() => handleDragEnter(idx)} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                  ...
                  <span style={{ background: projColor + '20', color: projColor, borderRadius: 4, padding: '2px 6px', fontSize: 10, fontWeight: 700, minWidth: 20, textAlign: 'center', marginTop: 3 }}>{idx + 1}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                      <span onClick={(e) => { e.stopPropagation(); toggleStatus(t); }} style={{ cursor: 'pointer', padding: '1px 7px', borderRadius: 4, fontSize: 10, fontWeight: 600, background: statusInfo.color + '18', color: statusInfo.color }}>{statusInfo.label}</span>
                    ...
                    {t.material && <div style={{ fontSize: 11, color: '#e6a23c', marginTop: 3 }}>🪵 {t.material}</div>}
      ...
      <Modal open={showAutoModal} onClose={() => setShowAutoModal(false)} title="Auto-generar procesos" width={440}>
        <p style={{ fontSize: 13, color: 'var(--t2)', marginBottom: 16, lineHeight: 1.5 }}>
          Genera Modelado, Planos, Optimización, Corte, Canteado y Mecanizado para un área. Si el área tiene materiales, crea un set de procesos por cada material.
        </p>
...
function FilterBtn({ children, active, onClick, color }) {
  return (
    <button onClick={onClick} style={{ padding: '5px 10px', borderRadius: 6, border: active ? `1.5px solid ${color}` : '1.5px solid var(--border)', background: active ? color + '15' : 'transparent', color: active ? color : 'var(--t2)', cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: 'inherit' }}>
      {children}
    </button>
  );
}
