...
export default function Planning({ projects, allAreas, userName }) {
  ...
  const filtered = useMemo(() => {
    return tasks
      ...
      .sort((a, b) => {
        // Sort by project, then area, then stage order
  ...
  async function save() {
    ...
    try {
      if (editing) { await updatePlanningTask(editing.id, { ...form, area_id: form.area_id||null }); }
      else { await createPlanningTask({ ...form, area_id: form.area_id||null, priority: tasks.length }); }
      setShowModal(false);
    } catch (e) { alert('Error: ' + e.message); }
  }
                ...
                <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
                  <button onClick={() => cycleStatus(t)}
                    style={{
                      ...
                      background: t.status==='done' ? statusInfo.color : t.status==='in_progress' ? statusInfo.color+'25' : 'transparent',
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', gap:8, fontSize:11, color:'var(--t2)', flexWrap:'wrap', alignItems:'center' }}>
                      ...
                      {t.material && <span style={{ background: matColor+'18', border:`1px solid ${matColor}40`, borderRadius:5, padding:'1px 8px', color: matColor, fontWeight:700, fontSize:11 }}>🪵 {t.material}</span>}
                    </div>
