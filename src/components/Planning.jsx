...
export default function Planning({ projects, allAreas, userName }) {
  ...
  const sortedProjects = useMemo(() =>
    [...projects]
      .filter((p) => !hidePausedProjects || (p.status !== 'paused' && p.status !== 'archived'))
      .sort((a,b) => (a.priority || 0) - (b.priority || 0)),
    [projects, hidePausedProjects]);
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <Btn variant="ghost" size="sm" onClick={() => { const d = new Date(weekMonday); d.setDate(d.getDate() + 7); setWeekMonday(d); }}>→</Btn>
          <Btn variant={hidePausedProjects ? 'secondary' : 'primary'} size="sm" onClick={() => setHidePausedProjects(!hidePausedProjects)}>
            {hidePausedProjects ? '⏸ Mostrar pausados' : '⏸ Ocultando pausados'}
          </Btn>
