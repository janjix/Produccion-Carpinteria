...
import {
  ...
  addAreaToWeek, clearAreaFromWeek,
...
export default function StaffView({ projects, allAreas, userName, currentStaff }) {
  ...
  const [wizardForceAreas, setWizardForceAreas] = useState([]);

  // v31: nuevo asistente para agregar área
  const [showAddAreaWizard, setShowAddAreaWizard] = useState(false);
  const [addAreaProjectId, setAddAreaProjectId] = useState('');
  const [addAreaAreaId, setAddAreaAreaId] = useState('');
  const [addAreaWeek, setAddAreaWeek] = useState('');

  // v31: asistente para limpiar área
  const [showClearWizard, setShowClearWizard] = useState(false);
  const [clearProjectId, setClearProjectId] = useState('');
  const [clearAreaId, setClearAreaId] = useState('');
  const [clearWeek, setClearWeek] = useState('');
  ...
  // v31: Agregar área a la semana
  function openAddAreaWizard() {
    setAddAreaProjectId('');
    setAddAreaAreaId('');
    setAddAreaWeek(weekStr);
    setShowAddAreaWizard(true);
  }

  async function confirmAddArea() {
    if (!addAreaAreaId || !addAreaWeek) return;
    setWizardBusy(true);
    try {
      const picked = new Date(addAreaWeek + 'T00:00:00');
      ...
      const weekMonday = `${y}-${m}-${d}`;

      const result = await addAreaToWeek({ areaId: addAreaAreaId, weekStartStr: weekMonday, staffList: staff });
      const area = allAreas.find((a) => a.id === addAreaAreaId);
      const proj = projects.find((p) => p.id === area?.project_id);
      await logActivity({
        project_id: area?.project_id, area_id: addAreaAreaId, action: 'area_added_to_week',
        description: `Área "${area?.name}" del proyecto "${proj?.name}" añadida a la semana del ${weekMonday}. ${result.created} tarea(s) creadas.`,
        user_name: userName,
      });

      setShowAddAreaWizard(false);
      let msg = `✓ ${result.created} tarea(s) creadas.`;
      if (result.skipped > 0) msg += `\n${result.skipped} omitida(s) (ya viven en otra semana activa).`;
      if (result.materialsMissing) msg += `\n⚠️ Esta área no tiene materiales configurados. Optimización/Corte/Canteado/Mecanizado no se generaron.`;
      alert(msg);
      await loadPlans();
  ...
  // v31: Limpiar área
  function openClearWizard() {
    setClearProjectId('');
    setClearAreaId('');
    setClearWeek(weekStr);
    setShowClearWizard(true);
  }

  async function confirmClearArea() {
    if (!clearAreaId || !clearWeek) return;
    setWizardBusy(true);
    try {
      const picked = new Date(clearWeek + 'T00:00:00');
      const dow = picked.getDay();
      const diff = dow === 0 ? -6 : 1 - dow;
      picked.setDate(picked.getDate() + diff);
      const y = picked.getFullYear();
      ...
      if (!confirm(`¿Borrar todas las tareas de esta área en la semana del ${weekMonday}?\n\nLas ya completadas quedan intactas. Otras semanas no se tocan.`)) {
        setWizardBusy(false);
        return;
      }

      const result = await clearAreaFromWeek({ areaId: clearAreaId, weekStartStr: weekMonday });
      const area = allAreas.find((a) => a.id === clearAreaId);
      const proj = projects.find((p) => p.id === area?.project_id);
      await logActivity({
        project_id: area?.project_id, area_id: clearAreaId, action: 'area_cleared_from_week',
        description: `Área "${area?.name}" del proyecto "${proj?.name}" limpiada de la semana del ${weekMonday}. ${result.removed} tarea(s) eliminadas.`,
        user_name: userName,
      });

      setShowClearWizard(false);
      alert(`✓ ${result.removed} tarea(s) eliminadas.`);
      await loadPlans();
    } catch (e) { alert('Error: ' + e.message); }
    setWizardBusy(false);
  }
  ...
  async function toggleItemStatus(item) {
    // v31: sin propagación. Solo actualiza el item y sincroniza el planning_task.
    ...
    try {
      ...
      if (item.project_id && item.stage_id) {
        try {
          const { data: tasks } = await supabase.from('planning_tasks').select('id, status')
          ...
          if (match && match.status !== next) {
            await updatePlanningTask(match.id, { status: next });
          }
        } catch (e) { console.error('planning_task sync error:', e); }
      }
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:8 }}>
        <div>
          ...
          <p style={{ margin:'4px 0 0', fontSize:13, color:'var(--t2)' }}>
            {isManager ? 'Vista de gerencia — agrega o limpia áreas por semana. Todo manual.' : `Planificación de ${currentStaff?.name||'tu semana'}`}
          </p>
        </div>
        {isManager && (
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            <Btn onClick={openAddAreaWizard} disabled={loading || wizardBusy}>➕ Agregar área a la semana</Btn>
            <Btn variant="secondary" onClick={openClearWizard} disabled={loading || wizardBusy}>🗑 Limpiar área de la semana</Btn>
          </div>
        )}
      </div>
...
function PersonPlan({ member, items, planId, staff, projectMap, areaMap, projectColorMap, allMaterialNames, allAreas, onToggle, onMoveDay, onMoveInDay, onRemove, onAdd, onExport, onClear, onToggleDone, onRefresh, onOpenActions }) {
  ...
  return (
    <div style={{ background:'var(--surface)', borderRadius:12, border:'1px solid var(--border)', overflow:'hidden' }}>
      ...
      {/* v31: Agregar área a la semana */}
      <Modal open={showAddAreaWizard} onClose={() => !wizardBusy && setShowAddAreaWizard(false)} title="Agregar área a la semana" width={500}>
        <p style={{ fontSize:13, color:'var(--t2)', marginBottom:14, lineHeight:1.5 }}>
          Elige un área. Todas sus tareas se crearán en la semana elegida en estado pendiente. Tú las repartes en los días manualmente.
        </p>
        <div style={{ marginBottom:14 }}>
          <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Proyecto</label>
          <select value={addAreaProjectId} onChange={(e) => { setAddAreaProjectId(e.target.value); setAddAreaAreaId(''); }}
            style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }}>
            <option value="">Selecciona un proyecto...</option>
            {[...projects].filter((p) => p.status !== 'archived').sort((a,b) => (a.priority||0) - (b.priority||0)).map((p) => (
              <option key={p.id} value={p.id}>{p.name}{p.client ? ` · ${p.client}` : ''}{p.status === 'paused' ? ' (pausado)' : ''}</option>
            ))}
          </select>
        </div>
        {addAreaProjectId && (
          <div style={{ marginBottom:14 }}>
            <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Área</label>
            <select value={addAreaAreaId} onChange={(e) => setAddAreaAreaId(e.target.value)}
              style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }}>
              <option value="">Selecciona un área...</option>
              {allAreas.filter((a) => a.project_id === addAreaProjectId).map((a) => (
        ...
        <div style={{ marginBottom:6 }}>
          <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Semana destino</label>
          <input type="date" value={addAreaWeek} onChange={(e) => setAddAreaWeek(e.target.value)}
            style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }} />
          <div style={{ fontSize:10, color:'var(--t2)', marginTop:4, fontStyle:'italic' }}>Cualquier día. Se normaliza al lunes de esa semana.</div>
        </div>
        <div style={{ display:'flex', justifyContent:'flex-end', gap:8, marginTop:20 }}>
          <Btn variant="secondary" onClick={() => setShowAddAreaWizard(false)} disabled={wizardBusy}>Cancelar</Btn>
          <Btn onClick={confirmAddArea} disabled={!addAreaAreaId || !addAreaWeek || wizardBusy}>
            {wizardBusy ? 'Creando...' : '✓ Agregar a la semana'}
          </Btn>
        </div>
      </Modal>

      {/* v31: Limpiar área */}
      <Modal open={showClearWizard} onClose={() => !wizardBusy && setShowClearWizard(false)} title="Limpiar área de la semana" width={500}>
        <p style={{ fontSize:13, color:'var(--t2)', marginBottom:14, lineHeight:1.5 }}>
          Elige el área y la semana. Todas sus tareas de esa semana se borrarán, excepto las ya completadas. Las de otras semanas no se tocan.
        </p>
        <div style={{ marginBottom:14 }}>
          <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Proyecto</label>
          <select value={clearProjectId} onChange={(e) => { setClearProjectId(e.target.value); setClearAreaId(''); }}
            style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }}>
            <option value="">Selecciona un proyecto...</option>
            {[...projects].sort((a,b) => (a.priority||0) - (b.priority||0)).map((p) => (
              <option key={p.id} value={p.id}>{p.name}{p.client ? ` · ${p.client}` : ''}</option>
        ...
        {clearProjectId && (
          <div style={{ marginBottom:14 }}>
            <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Área</label>
            <select value={clearAreaId} onChange={(e) => setClearAreaId(e.target.value)}
              style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }}>
              <option value="">Selecciona un área...</option>
              {allAreas.filter((a) => a.project_id === clearProjectId).map((a) => (
                <option key={a.id} value={a.id}>{a.name}</option>
              ))}
            </select>
          </div>
        ...
        <div style={{ marginBottom:6 }}>
          <label style={{ display:'block', marginBottom:6, fontSize:12, fontWeight:700, color:'var(--t2)', textTransform:'uppercase', letterSpacing:'0.04em' }}>Semana</label>
          <input type="date" value={clearWeek} onChange={(e) => setClearWeek(e.target.value)}
            style={{ width:'100%', padding:'10px 12px', borderRadius:8, border:'1.5px solid var(--border)', background:'var(--bg)', color:'var(--t1)', fontSize:14, fontFamily:'inherit' }} />
        </div>
        <div style={{ display:'flex', justifyContent:'flex-end', gap:8, marginTop:20 }}>
          <Btn variant="secondary" onClick={() => setShowClearWizard(false)} disabled={wizardBusy}>Cancelar</Btn>
          <Btn onClick={confirmClearArea} disabled={!clearAreaId || !clearWeek || wizardBusy}>
            {wizardBusy ? 'Borrando...' : '🗑 Borrar tareas'}
          </Btn>
        </div>
      </Modal>
    </div>
