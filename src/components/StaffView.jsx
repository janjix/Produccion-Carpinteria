...
import { STAGES, DAYS_OF_WEEK, PROJECT_COLORS, MATERIAL_COLORS, MANAGER_CODES } from '../lib/constants.js';
import {
  ...
  deleteCompletedWeeklyItems, updatePlanningTask,
...
export default function StaffView({ projects, allAreas, userName, currentStaff }) {
  ...
  const [selectedStaff, setSelectedStaff] = useState(null); // for managers: which person to view
  const [showAddModal, setShowAddModal] = useState(false);
  const [addTarget, setAddTarget] = useState(null); // staff id to add item to
  ...
  async function generateWeeklyPlan() {
    setLoading(true);
    try {
      const pending = planningTasks.filter((t) => t.status !== 'done');
      const today = getTodayDayIndex();
      const usedTaskIds = new Set();

      for (const member of staff) {
        if (plans.find((p) => p.staff_id === member.id)) continue;
        ...
        try {
          ...
          if (myPrevPlan) {
            ...
            for (const pi of prevItems.filter((i) => i.status !== 'done')) {
              items.push({ ...pi, plan_id: plan.id, id: undefined, day_of_week: today, sort_order: sort++, status: pi.status });
              dayCount[today]++;
            }
          }
        } catch (e) { /* ok */ }

        // New tasks from planning
        const relevant = pending.filter((t) => t.stage && memberProcs.includes(t.stage) && !usedTaskIds.has(t.id));
        for (const task of relevant) {
          const alreadyCarried = items.some((i) =>
          ...
          if (alreadyCarried) { usedTaskIds.add(task.id); continue; }
          // Assign to today or next available day
          let day = today;
          for (let d = today; d <= 6; d++) { if (dayCount[d] < (d===6?3:5)) { day=d; break; } }
          items.push({
            ...
            material: task.material||'', day_of_week: day, sort_order: sort++,
            status: 'pending', notes: task.description||'', is_admin:false, is_general:false,
          });
          usedTaskIds.add(task.id); dayCount[day]++;
        }

        if (member.code === 'AA') {
          items.push({ plan_id:plan.id, staff_id:member.id, project_id:null, area_id:null, process:'Mantenimiento de Máquinas', material:'', day_of_week:5, sort_order:sort++, status:'pending', notes:'', is_admin:false, is_general:true });
        }
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:8 }}>
        ...
        {isManager && <Btn onClick={generateWeeklyPlan} disabled={loading}>{loading?'Generando...':'⚡ Generar semana'}</Btn>}
      </div>
                  ...
                  {dayItems.map((item, itemIdx) => {
                    ...
                    return (
                      <div key={item.id} style={{
                        padding:'5px 6px', borderRadius:5, fontSize:10, lineHeight:1.3,
                        background: item.status==='done' ? '#2dcc9f06' : mc ? mc+'08' : 'var(--bg)',
                        borderLeft:`3px solid ${borderColor}`,
                        border:`1px solid ${mc?mc+'25':'transparent'}`,
                        ...
                        {mc && item.material && (
                          <div style={{ background:mc+'18', border:`1px solid ${mc}30`, borderRadius:3, padding:'1px 4px', marginBottom:2, fontSize:8, fontWeight:700, color:mc }}>
                            🪵 {item.material}
                          </div>
                        )}
