                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {doneCount > 0 && (
                      <Btn variant="ghost" size="xs" onClick={async () => {
                        if (!confirm(`¿Eliminar ${doneCount} tareas completadas de ${member.name}?`)) return;
                        if (memberData?.planId) { await deleteCompletedWeeklyItems(memberData.planId); await loadPlans(); }
                      }}>🗑 Limpiar</Btn>
                    )}
                  </div>
                </div>
