...
export default function ProjectDetail({ project, onBack, userName }) {
  ...
  async function toggleStage(stageId) {
    ...
    // Sync: when completing a stage, also mark matching planning tasks as done and propagate
    if (newVal) {
      try {
        ...
        for (const task of matching) {
          await updatePlanningTask(task.id, { status: 'done' });
          ...
          if (created.length > 0) {
            await logActivity({
              project_id: project.id, action: 'auto_propagated',
              description: `${created.length} proceso(s) creado(s) tras completar "${stage.label}" en ${currentArea.name}`,
              user_name: 'Sistema',
            });
          }
        }
                  ...
                  <div key={stage.id}>
                    <div
                      ...
                      <span style={{
                        ...
                        border: `2px solid ${checked ? stage.color : 'var(--border)'}`, background: checked ? stage.color : 'transparent',
                      ...
                      <span onClick={(e) => { e.stopPropagation(); setExpandedComment(isExpanded ? null : stage.id); }}
                        style={{ fontSize: 11, color: 'var(--t2)', padding: '2px 6px', borderRadius: 4, cursor: 'pointer' }}>
                        {isExpanded ? 'Cerrar' : '💬'}
                      </span>
                    </div>
                    {isExpanded && (
