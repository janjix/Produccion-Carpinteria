...
import {
  useAreas, useFurniture, useAreaMaterials,
...
export default function ProjectDetail({ project, onBack, userName, planningTasks = [] }) {
  ...
  async function saveArea() {
    ...
    try {
      if (editingArea) { await updateArea(editingArea.id, { name: areaForm.name, mecanizados_enabled: areaForm.mecanizados }); }
      else { const a = await createArea({ project_id: project.id, name: areaForm.name, mecanizados_enabled: areaForm.mecanizados, sort_order: areas.length }); await logActivity({ project_id: project.id, area_id: a.id, action: 'area_created', description: `Área "${areaForm.name}" creada en ${project.name}`, user_name: userName }); setSelectedAreaId(a.id); }
              ...
              {checklistOrder.map((sid) => {
                ...
                return (
                  <div key={sid} style={{
                    ...
                    {/* Show materials for this stage */}
                    {tasks.length > 0 && tasks.some((t) => t.material) && (
                      <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                        {tasks.map((t) => t.material ? (
                          <span key={t.id} style={{
                            fontSize: 9, padding: '2px 6px', borderRadius: 4, fontWeight: 600,
                            background: t.status === 'done' ? '#2dcc9f15' : '#e6a23c15',
                            color: t.status === 'done' ? '#2dcc9f' : '#e6a23c',
                            border: `1px solid ${t.status === 'done' ? '#2dcc9f30' : '#e6a23c30'}`,
                          }}>🪵 {t.material} {t.status === 'done' ? '✓' : ''}</span>
                        ) : null)}
                      </div>
                    )}
                  </div>
...
function MaterialsSection({ area, projectId, userName }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', padding: 16, marginBottom: 16 }}>
      ...
      <p style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 10 }}>Cada material genera procesos independientes (Optimización, Corte, Canteado, Mecanizado).</p>
