...
function MaterialsSection({ area, projectId, userName }) {
  ...
  async function addMaterial() {
    ...
    try {
      await createAreaMaterial({ area_id: area.id, name: newMat.trim(), sort_order: materials.length });
      setNewMat('');
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <h4 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--t1)' }}>🪵 Materiales del área</h4>
          <Badge>{materials.length}</Badge>
        </div>
      </div>
      <p style={{ fontSize: 11, color: 'var(--t2)', marginBottom: 10 }}>
        Agrega los materiales antes de auto-generar la planificación. Cada material genera su propio set de procesos.
      </p>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {materials.map((mat) => (
            <span key={mat.id} style={{
              ...
              background: '#e6a23c15', border: '1px solid #e6a23c30',
              fontSize: 12, fontWeight: 500, color: '#e6a23c',
            }}>
              🪵 {mat.name}
              <span onClick={() => removeMaterial(mat)} style={{ cursor: 'pointer', fontSize: 10, opacity: 0.7 }}>✕</span>
            </span>
          ))}
        </div>
      )}
