...
import { STAGES, AREA_CHECKLIST_STAGES, MATERIAL_PROCESSES, MATERIAL_COLORS } from '../lib/constants.js';

const checklistStages = STAGES.filter(s => AREA_CHECKLIST_STAGES.includes(s.id));
const materialStageIds = MATERIAL_PROCESSES.map((p) => p.id);
                        ...
                        {projAreas.map((area) => {
                          ...
                          return (
                            <div key={area.id} style={{ background: 'var(--bg)', borderRadius: 10, border: '1px solid var(--border)', padding: 14 }}>
                              ...
                              {/* Material breakdown */}
                              {(() => {
                                const areaTasks = planningTasks.filter((t) => t.project_id === proj.id && t.area_id === area.id && t.material);
                                if (areaTasks.length === 0) return null;
                                const byMaterial = {};
                                areaTasks.forEach((t) => {
                                  if (!byMaterial[t.material]) byMaterial[t.material] = {};
                                  byMaterial[t.material][t.stage] = t.status;
                                });
                                return (
                                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed var(--border)' }}>
                                    <div style={{ fontSize: 10, color: 'var(--t2)', fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Materiales</div>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                                      {Object.entries(byMaterial).map(([matName, stageStatuses]) => {
                                        const mc = getMaterialColor(matName, allMaterialNames);
                                        return (
                                          <div key={matName} style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                            <span style={{
                                              background: mc + '20', border: `1px solid ${mc}40`,
                                              padding: '3px 8px', borderRadius: 5,
                                              fontSize: 10, fontWeight: 700, color: mc,
                                              minWidth: 100, display:'inline-flex', alignItems:'center', gap:3,
                                            }}>🪵 {matName}</span>
                                            <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                                              {materialStageIds.map((sid) => {
                                                const st = stageStatuses[sid];
                                                if (!st) return null;
                                                ...
                                                return (
                                                  <span key={sid} style={{
                                                    ...
                                                    background: isDone ? stage.color + '20' : isProg ? '#4a9eff15' : 'var(--surface)',
                                                    color: isDone ? stage.color : isProg ? '#4a9eff' : 'var(--t2)',
                                                    border: `1px solid ${isDone ? stage.color + '40' : isProg ? '#4a9eff30' : 'var(--border)'}`,
                                                  }}>
                                                    {stage.icon} {stage.label} {isDone ? '✓' : isProg ? '◉' : '○'}
                                                  </span>
                                                );
                                              })}
                                            </div>
