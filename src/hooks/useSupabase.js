...
export async function applyScheduledWeeks() {
  ...
  try {
    ...
    for (const task of tasks) {
      try {
        // syncTaskToWeeklyPlan ya tiene su propia guardia contra duplicados
        await syncTaskToWeeklyPlan(task, staffList, { force: true });
        applied.count++;
        if (task.area_id && !applied.areas.includes(task.area_id)) applied.areas.push(task.area_id);
      } catch (e) {
        console.error('applyScheduledWeeks item error:', e);
      }
    }
    return applied;
  } catch (e) {
