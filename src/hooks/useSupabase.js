...
export async function syncTaskToWeeklyPlan(task, staffList) {
  ...
  try {
    ...
    if (found) {
      // Already in the week — but make sure its status matches the task (e.g. blocked→pending)
      if (found.status !== (task.status || 'pending')) {
        await supabase.from('weekly_plan_items').update({ status: task.status || 'pending' }).eq('id', found.id);
      }
      return;
    }
      ...
      for (const task of tasks) {
        ...
        if (!allDepsDone) {
          ...
          try {
            ...
            if (matchItems) {
              for (const item of matchItems) {
                if ((item.material || '') === (task.material || '') && item.status !== 'blocked' && item.status !== 'done') {
