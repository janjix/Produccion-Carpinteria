...
// When a planning task is marked done, find successor processes and create
// planning tasks for the responsible staff member automatically.
...
export async function propagateNextProcesses(completedTask, allPlanningTasks, staffList) {
  ...
  for (const nextProc of successors) {
    // Check if task already exists for this area + stage + material
    ...
    // Find the responsible staff member for this process
    let assignedStaffId = null;
    for (const member of staffList) {
      const procs = member.default_processes || [];
      if (procs.includes(nextProc)) {
    ...
    try {
      ...
      created.push({ ...result, _assigned_staff_id: assignedStaffId });
    } catch (e) {
