...
export async function moveWeeklyItemToWeek(itemId, { deltaWeeks = 1, targetStaffId = null, fromWeekStart }) {
  try {
    ...
    // CLAVE: actualizar la planning_task general para que su scheduled_week_start
    // apunte a la semana destino. Sin esto, la sincronizacion volveria a crear la tarea
    // en la semana original la proxima vez que corra applyScheduledWeeks o syncTaskToWeeklyPlan.
    if (item.project_id && item.stage_id) {
      try {
        const { data: tasks } = await supabase.from('planning_tasks').select('id, area_id, material')
        ...
        if (match) {
          const updates = { scheduled_week_start: toWeekStart };
          if (targetStaffId) updates.assigned_to_id = newStaffId;
          await supabase.from('planning_tasks').update(updates).eq('id', match.id);
        }
      } catch (e) { console.error('No se pudo actualizar planning_task al mover:', e); }
    }
...
function getCurrentMonday() {
  ...
  // Componentes locales, no UTC. Ver comentario en toDateStr de StaffView.
...
export async function propagateNextProcesses(completedTask, allPlanningTasks, staffList) {
  // NEUTRALIZADO en v31: no propaga nada. El check solo marca la tarea como hecha.
  return [];
...
export async function syncTaskToWeeklyPlan(task, staffList, opts = {}) {
  // NEUTRALIZADO en v31: no sincroniza automáticamente. Las tareas entran a la semana
  // solo cuando David usa "Agregar área a la semana".
  return;
  // eslint-disable-next-line no-unreachable
  ...
  // Respetar la semana programada. La tarea solo se sincroniza en la semana actual
  // si su scheduled_week_start coincide con la actual, o si es del pasado y aun no se
  // aplico. Si es futura, esperar. Si ya se movio a otra semana, respetar ese destino.
  if (!opts.force) {
    let scheduled = task.scheduled_week_start;
    // Fallback: si la tarea no trae la columna, leer del area
    ...
    if (scheduled) {
      ...
      // Solo insertar si la semana programada llego (<= actual).
      // Si es futura, esperar a applyScheduledWeeks.
      if (scheduledStr > currentMon) {
        console.log('[syncTaskToWeeklyPlan] Saltando, programada para', scheduledStr, task.stage, task.material || '');
        return;
      }
    }
  }
...
export async function generateAreaPipeline({ project, area, materials, staffList, existingTasks }) {
  // NEUTRALIZADO en v31: crear un área ya no genera pipeline automáticamente.
  return [];
  // eslint-disable-next-line no-unreachable
...
export async function assignProjectToWeek({ projectId, weekStartStr, staffList, forceAreaIds = [] }) {
  // NEUTRALIZADO en v31: reemplazado por addAreaToWeek (una área a la vez).
  return { assignedByStaff: {}, skipped: 0, total: 0, futureSkipped: 0 };
  // eslint-disable-next-line no-unreachable
...
export async function applyScheduledWeeks() {
  // NEUTRALIZADO en v31: sin aplicación automática. Todo se hace desde "Agregar área".
  ...
  try {
    ...
    // 4) Items existentes en CUALQUIER semana (no solo actual): asi respetamos
    //    tareas que ya fueron movidas a otra semana con moveWeeklyItemToWeek.
    const projectIdsForCheck = [...new Set(activeTasks.map((t) => t.project_id).filter(Boolean))];
    let existingItemsAllWeeks = [];
    if (projectIdsForCheck.length > 0) {
      const { data } = await supabase.from('weekly_plan_items')
        .select('id, plan_id, project_id, area_id, stage_id, material, status, process')
        .in('project_id', projectIdsForCheck)
        .not('status', 'in', '(dismissed)');
      existingItemsAllWeeks = data || [];
    }

    // 5) Items dismissed en la semana actual: no re-insertar aunque toque
    const planIds = Object.values(planByStaffId).map((p) => p.id);
    let dismissedThisWeek = [];
    if (planIds.length > 0) {
      const { data } = await supabase.from('weekly_plan_items')
        .select('project_id, area_id, stage_id, material')
        .in('plan_id', planIds)
        .eq('status', 'dismissed');
      dismissedThisWeek = data || [];
    }
    const dismissedKey = new Set();
    dismissedThisWeek.forEach((i) => {
      dismissedKey.add(`${i.project_id||''}|${i.area_id||''}|${i.stage_id||''}|${i.material||''}`);
    });

    const norm = (s) => (s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
    const existsAnywhere = (projectId, areaId, stage, material, stageLabel) => {
      return existingItemsAllWeeks.some((i) => {
    ...
    for (const task of activeTasks) {
      ...
      if (existsAnywhere(task.project_id, task.area_id, task.stage, task.material, label)) continue;
...
// ─── v31: Nueva mecánica manual ───
// Agrega TODAS las tareas de un área a una semana específica.
export async function addAreaToWeek({ areaId, weekStartStr, staffList }) {
  const result = { created: 0, skipped: 0, byStaff: {}, materialsMissing: false };
  try {
    const { data: area, error: aerr } = await supabase.from('areas').select('*').eq('id', areaId).single();
    if (aerr || !area) throw new Error('Área no encontrada');
    const projectId = area.project_id;

    const materials = await getAreaMaterialsForArea(areaId);
    const materialNames = materials.map((m) => m.name);
    if (materialNames.length === 0) result.materialsMissing = true;

    async function ensurePlan(staffId) {
      const { data: plans } = await supabase.from('weekly_plans').select('*').eq('week_start', weekStartStr).eq('staff_id', staffId);
      let plan = (plans || [])[0];
      if (!plan) plan = await createWeeklyPlan({ staff_id: staffId, week_start: weekStartStr });
      return plan;
    }

    const stagesToGenerate = [
      { id: 'diseno', perMaterial: false },
      { id: 'revision_diseno', perMaterial: false },
      { id: 'creacion_partidas', perMaterial: false },
      { id: 'req_materiales', perMaterial: false },
      { id: 'req_herrajes', perMaterial: false },
      { id: 'req_sistema', perMaterial: false },
      { id: 'compra_materiales', perMaterial: false },
      { id: 'modelado', perMaterial: false },
      { id: 'planos', perMaterial: false },
      { id: 'optimizacion', perMaterial: true },
      { id: 'corte', perMaterial: true },
      { id: 'canteado', perMaterial: true },
      { id: 'supervision_canteado', perMaterial: false },
      { id: 'mecanizado', perMaterial: true },
      { id: 'ensamblaje', perMaterial: false },
      { id: 'herrajes', perMaterial: false },
      { id: 'supervision_ensamblaje', perMaterial: false },
      { id: 'despacho_materiales', perMaterial: false },
      { id: 'embalaje', perMaterial: false },
      { id: 'instalacion', perMaterial: false },
      { id: 'supervision_instalacion', perMaterial: false },
      { id: 'despacho_admin', perMaterial: false },
    ];

    const { data: existingItems } = await supabase.from('weekly_plan_items')
      .select('project_id, area_id, stage_id, material, status')
      .eq('area_id', areaId)
      .in('status', ['pending', 'in_progress']);

    const existsAlive = (stageId, material) => {
      return (existingItems || []).some((i) => {
        if ((i.material || '') !== (material || '')) return false;
        return i.stage_id === stageId;
      });
    };
    ...
    for (const stage of stagesToGenerate) {
      ...
      if (!stageInfo) continue;
      const label = stageInfo.label;

      const primaryCode = PROCESS_PRIMARY_OWNER[stage.id];
      let assignee = null;
      if (primaryCode) {
        assignee = staffList.find((m) => m.code === primaryCode);
      }
      if (!assignee) {
        assignee = staffList.find((m) => (m.default_processes || []).includes(stage.id));
      }
      if (!assignee) continue;

      const plan = await ensurePlan(assignee.id);
      const hours = PROCESS_DEFAULT_HOURS[stage.id] ?? null;

      const materialsForThisStage = stage.perMaterial ? materialNames : [''];
      if (stage.perMaterial && materialsForThisStage.length === 0) continue;

      for (const mat of materialsForThisStage) {
        if (existsAlive(stage.id, mat)) { result.skipped++; continue; }

        cursorByStaff[assignee.id] = (cursorByStaff[assignee.id] || 0);
        const day = (cursorByStaff[assignee.id] % 6) + 1;
        cursorByStaff[assignee.id]++;

        toInsert.push({
          plan_id: plan.id, staff_id: assignee.id,
          project_id: projectId, area_id: areaId,
          process: label, stage_id: stage.id,
          material: mat,
          day_of_week: day, sort_order: cursorByStaff[assignee.id],
          status: 'pending',
          notes: '',
          estimated_hours: hours,
          is_admin: false, is_general: false,
        });
        result.byStaff[assignee.id] = (result.byStaff[assignee.id] || 0) + 1;
        result.created++;

        // Asegurar planning_task general para mantener consistencia
        try {
          const { data: existTasks } = await supabase.from('planning_tasks')
            .select('id, status')
            .eq('project_id', projectId).eq('area_id', areaId)
            .eq('stage', stage.id);
          const dupTask = (existTasks || []).find((t) => (t.material || '') === (mat || ''));
          if (!dupTask) {
            const matSuffix = mat ? ` (${mat})` : '';
            const areaName = area.name || '';
            await createPlanningTask({
              project_id: projectId, area_id: areaId,
              title: `${label} — ${areaName}${matSuffix}`,
              stage: stage.id, status: 'pending',
              priority: 0, material: mat || '', description: '',
              estimated_hours: hours,
              scheduled_week_start: weekStartStr,
              assigned_to_id: assignee.id,
            });
          }
        } catch (e) { console.error('planning_task upsert error:', e); }
      }
    }
    ...
    console.error('addAreaToWeek error:', e);
    throw e;
  }
}

// Borra TODOS los weekly_plan_items de un área en una semana específica.
export async function clearAreaFromWeek({ areaId, weekStartStr }) {
  try {
    const { data: plans } = await supabase.from('weekly_plans').select('id').eq('week_start', weekStartStr);
    const planIds = (plans || []).map((p) => p.id);
    if (planIds.length === 0) return { removed: 0 };

    const { data: toDelete } = await supabase.from('weekly_plan_items')
      .select('id')
      .eq('area_id', areaId)
      .in('plan_id', planIds)
      .neq('status', 'done');
    ...
    const { error } = await supabase.from('weekly_plan_items').delete().in('id', ids);
    if (error) throw error;
    return { removed: ids.length };
  } catch (e) {
    console.error('clearAreaFromWeek error:', e);
    throw e;
  }
}
