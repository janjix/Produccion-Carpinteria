...
// When called: creates Diseño + Revisión + Creación Partidas + Modelado + Planos + Req. Mat + Req. Herr
// + (per material) Optimización, Corte, Canteado, Mecanizado + QC + Herrajes + Ensamblaje + Embalaje
// + Instalación + Mediciones + Despacho admin.
  ...
  for (const stageId of ALL_PIPELINE_STAGES) {
    ...
    for (const matObj of materialsToUse) {
      ...
      try {
        ...
        created.push(task);
        // Sync to staff's weekly plan only if NOT blocked
        if (initialStatus !== 'blocked' && staffList) {
